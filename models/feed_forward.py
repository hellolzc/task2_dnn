...
class FeedForward(object):
    ...
    def __init__(self, n_input, n_output, n_hidden, n_units, active_funcs,
                 output_layer = 'soft_max', keep_prob=1):
        '''
        n_input: the dim of input
        n_hidden: the numbers of hidden layers, int
        n_units: lists, the neural units of every layer. If len(n_nuits) == 1,
                 every layer uses the same numbers of units
        ...
        for i in range(0, n_hidden +1):
            ...
            b_name = 'b_' + str(i)
            if i == 0:
                self.h = self.active_funcs(tf.matmul(self.x, self.weights[w_name]) +
                                            self.weights[b_name])
                if keep_prob != 1:
                    self.h = tf.nn.dropout(self.h, keep_prob)

            elif i > 0 and i < n_hidden:
                self.h = self.active_funcs(tf.matmul(self.h, self.weights[w_name]) +
                                           self.weights[b_name])
            else:
                self.h = tf.matmul(self.h, self.weights[w_name]) + self.weights[b_name]
        if output_layer == 'softmax':
            self.output = tf.nn.softmax(self.h)
            self.cost = tf.nn.softmax_cross_entropy_with_logits(labels=self.labels,
                                                                logits=self.output)
