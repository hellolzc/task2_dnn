...
if exist('viewangle','var')==0
    viewangle = 0;
end

% Fetch data from CSV file
numericData = csvread([docname,'.csv']);
fpsfactor = 4;
...
[ x, y, z ] = check_points(x, y, z, numframes);
prepare_video( docname, viewangle, x, y, z, numframes);
end


function [ x, y, z ] = check_points(x, y, z, numframes)
...
function prepare_video( docname, viewangle, x, y, z, numframes)
%UNTITLED 此处显示有关此函数的摘要
%   此处显示详细说明
% Prepare video
fig1 = figure(1);
...
vid = VideoWriter([docname,'_',num2str(viewangle),'.avi']);
vid.Quality = 100;
vid.FrameRate = 25;
open(vid);
