...
def get_mini_batch(sess, coord, dnn, file_list, l, r, batch_size, num_threads, num_epoches):
    feats, labels = read_and_decode(file_list, dnn.n_input/(l+r+1), dnn.n_output, num_epoches)
    sess.run(tf.local_variables_initializer())
    sfeats = splice_feats(feats, l, r)
    slice_queue = tf.RandomShuffleQueue(capacity=batch_size*50,
                                        min_after_dequeue=0,
                                        dtypes=['float', 'float'],
                                        shapes=[[dnn.n_input,], [dnn.n_output,]])
    batch_x, batch_y = slice_queue.dequeue_many(batch_size)
    enqueue = [slice_queue.enqueue_many([sfeats, labels])]*num_threads
    qr = tf.train.QueueRunner(slice_queue, enqueue)
    qr.create_threads(sess, coord=coord, start=True)
    return batch_x, batch_y

def train(sess, coord, dnn, batch_size, batch_x, batch_y, num_threads, training, clear_stop):
...
'''
    ...
    try:
        while not coord.should_stop():
            ...
            cost = dnn.partial_fit(bx, by, training)
...
def main(_):
    ...
    halving_factor = FLAGS.halving_factor# halve: vtr	(decrease by half)
    ...
    dnn = ff.FeedForward(input_dim*(l+r+1), output_dim, num_layers, [num_units],
                         active_func, output_layer='linear', keep_prob=keep_prob)
    ...
    train_batch_x, train_batch_y = get_mini_batch(sess, coord, dnn, train_list, l, r,
                                                 batch_size,num_threads, 1)
    dev_batch_x, dev_batch_y = get_mini_batch(sess, coord, dnn, dev_list, l, r,
                                            batch_size, num_threads, 1)
    ...
    cur_tr_costs = train(sess, coord, dnn, batch_size, train_batch_x, train_batch_y,
                         num_threads, True, True)
    etime = datetime.datetime.now()
    print 'Training cost: ', cur_tr_costs, etime - stime

    cur_cv_costs = train(sess, coord, dnn, batch_size, dev_batch_x, dev_batch_y,
    ...
    best_model = saver.save(dnn.sess, save_path+'/train_iter'+str(iter_num)+\
                '_lr_' + str(lr) + '_tr_'+ str(cur_tr_costs)+'_cv_'+str(cur_cv_costs))
